...
def _get_fallback_firecrawl_client():
    ...
    # 使用模块顶层的 lazy proxy，与 _get_firecrawl_client() 风格一致
...
def _run_async_safely(coro):
    ...
    - 若已在 running loop 中（不应发生在同步 web_search_tool 调用中，
      但做防御处理），将协程提交到独立线程执行避免死锁。
    """
    try:
    ...
    # 已存在 running loop —— 起独立线程跑一个新 loop
    def _runner():
        return asyncio.run(coro)
...
async def web_extract_tool(
    ...
    Args:
        ...
        query (Optional[str]): 原始检索关键词。当传入的 URL 全部抓取失败时，用它调用 web_search 拉取新候选重试。
    ...
    try:
        ...
        # Determine Firecrawl formats once (shared by main loop and replacement loop)
        if format == "markdown":
